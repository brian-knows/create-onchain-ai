...
async function init() {
  ...
  const defaultProjectName = "onchainkit-ai-app";

  let result: prompts.Answers<
    "projectName" | "packageName" | "clientKey" | "smartWallet" | "brianKey"
  >;
      ...
      [
        ...
        },
        {
          ...
          message: pc.reset(
            `Enter your ${createClickableLink(
              "Brian API Key:",
              "https://www.brianknows.org/app/settings"
            )}`
          ),
          validate: (value) => (value ? true : "Brian API Key is required"),
        },
        {
          type: "toggle",
          name: "smartWallet",
  ...
  const { projectName, packageName, clientKey, smartWallet, brianKey } = result;
  ...
  console.log(`${pc.greenBright("\u2713")} ${pc.blueBright(`Brian`)}`);
